...
SKILL_DIR="$(cd "$QA_DIR/../../../skills/en/huawei-cloud-account-onboarding" && pwd)"
...
check_skill_layout() {
  ...
  if git -C "$ROOT" ls-files --error-unmatch \
      "skills/en/huawei-cloud-account-onboarding/scripts/node_modules" >/dev/null 2>&1; then
  ...
  [[ "$body_lines" -le 35 ]] \
    || fail "SKILL.md body is $body_lines non-empty lines; keep it an entry point (<=35)"
}
...
doc = root / "docs/skills/en/huawei-cloud-account-onboarding.md"
...
labels = {
    ...
    "企业实名认证": "enterprise verification",
}
missing = []
for op, c in (spec.get("operations") or {}).items():
    ...
    for field, fs in (c.get("response_fields") or {}).items():
        ...
        for value, label in (fs.get("enum") or {}).items():
            expected_label = labels.get(label, label)
            if f"`{value}`" not in commands or expected_label not in commands:
                missing.append(f"{op}.{field}: enum {value} ({expected_label})")
...
# 门禁事实必须同时出现在概念层
if "does not check verification status" not in concepts:
...
check_scripts() {
  ...
  rg -q 'Single use only' <<<"$out" || fail "render-qr.ts must warn the QR is single-use"
