...
SKILL_DIR="$(cd "$QA_DIR/../../../skills/en/huawei-cloud-cost-estimation" && pwd)"
...
main_lower = " ".join(main.lower().split())
concepts = skill.joinpath("references/lifecycle/concepts.md").read_text(encoding="utf-8")
required = ["## execution pace", "at most three", "longer chains", "two or three commands per round", "ask whether to continue", "uninterrupted execution", "--dryrun"]
missing = [token for token in required if token not in main_lower]
if missing:
