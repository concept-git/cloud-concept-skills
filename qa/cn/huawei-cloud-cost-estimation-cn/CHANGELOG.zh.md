# huawei-cloud-cost-estimation-cn 更新日志
...
- 将标签堆叠的 Call Budget 压缩为两句易懂的 Execution Pace 规则
- 保持小任务直达、大任务每轮 2–3 条、友好询问继续及创建安全门禁不变

## 3.2.0 - 2026-08-01

### 新增

- 增加 Call Budget：不超过 3 条 `hcloud` 命令直接完成；更长命令链按 2–3 条可复用阶段交付
- 每个进度检查点必须报告已完成证据、当前可用结果、剩余命令数与下一阶段产出
- 新增 eval #19 与渐进执行静态门禁

### 变更

- 查询结果去重复用，独立只读命令可并行，同计费模式的多产品合并询价
- 用户明确要求一次完成或继续时不重复打断，但歧义、未知费用、`--dryrun` 与正式创建确认门禁保持不变
- 生命周期批次允许分段收集证据，最终确认仍覆盖已全部 dry-run 的完整批次
...
- `docs/skills/huawei-cloud-cost-estimation-cn.md`；`docs/catalog.yml` 索引；README 安装说明
