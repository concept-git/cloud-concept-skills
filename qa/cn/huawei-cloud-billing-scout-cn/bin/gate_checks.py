...
def check_layout() -> None:
    ...
    if list(SKILL_DIR.rglob(".DS_Store")):
        raise SystemExit("FAIL: remove .DS_Store from skills/huawei-cloud-billing-scout-cn")
...
def _check_frontmatter(frontmatter: dict, *, version: str | None = None) -> None:
    if frontmatter.get("name") != "huawei-cloud-billing-scout-cn":
    ...
    if "cloud-concept-skills/tree/main/skills/cn/huawei-cloud-billing-scout-cn" not in openclaw.get("homepage", ""):
...
def _check_catalog_entry(entry: dict | None, version: str) -> None:
    if not entry:
        raise SystemExit("FAIL: docs/catalog.yml missing huawei-cloud-billing-scout-cn")
    if entry.get("path") != "skills/cn/huawei-cloud-billing-scout-cn" or entry.get("qa") != "qa/cn/huawei-cloud-billing-scout-cn":
...
def _check_evals(data: dict, semantic_entities: set[str], contracts: dict) -> int:
    if data.get("skill_name") != "huawei-cloud-billing-scout-cn":
...
def check_docs_and_evals() -> tuple[int, int]:
    ...
    entry = next((item for item in catalog.get("skills", []) if item.get("id") == "huawei-cloud-billing-scout-cn"), None)
