...
SKILL_DIR="$(cd "$QA_DIR/../../../skills/cn/huawei-cloud-account-onboarding-cn" && pwd)"
...
check_skill_layout() {
  ...
  if git -C "$ROOT" ls-files --error-unmatch \
      "skills/cn/huawei-cloud-account-onboarding-cn/scripts/node_modules" >/dev/null 2>&1; then
...
entry = next(x for x in catalog.get("skills", []) if x.get("id") == "huawei-cloud-account-onboarding-cn")
...
doc = root / "docs/skills/cn/huawei-cloud-account-onboarding-cn.md"
...
printf 'OK: huawei-cloud-account-onboarding-cn validation passed\n'
